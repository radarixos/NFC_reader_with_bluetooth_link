...
void setup() {
  ...
  tft.print("Radarixos, ver. 0.3a");bluetooth.println("Radarixos, ver. 0.3a");
  tft.setCursor(3,90);
  tft.setTextSize(1);
...
void loop() {
  ...
  uspech = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, &uid[0], &uidLength, &key);
  ...
  if (uspech) {
    ...
    for (uint8_t i = 0; i < uidLength; i++) {
      ...
      tft.setTextColor(ST77XX_WHITE);
      tft.print(uid[i], HEX); bluetooth.print(uid[i], HEX);
     }
